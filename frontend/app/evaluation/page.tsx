        ...
        <div className="bg-white border-2 border-dashed border-indigo-200 rounded-3xl p-6 md:p-8 bg-gradient-to-br from-indigo-50/15 via-white to-white space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-mono focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all shadow-inner bg-white font-medium"
              disabled={running}
            />
          ...
          <div className="flex items-end justify-between pt-4 border-t border-slate-100">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-24 px-3 py-2.5 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm bg-white"
                disabled={running}
              />
            </div>
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 flex flex-col h-[500px]">
          <div className="border-b border-slate-100 pb-3 mb-4 flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100 uppercase tracking-wide">Live SWR Feed</span>
          </div>
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-3">
            {(!finalRuns || finalRuns.length === 0) ? (
              ...
              finalRuns.map((r: any) => (
                <div 
                  key={r.id} 
                  className="p-4 rounded-xl border border-slate-200/85 hover:border-slate-300 hover:bg-slate-50/50 transition-colors flex flex-col gap-2.5"
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className={`flex items-center gap-1 text-[11px] font-extrabold uppercase tracking-wide ${
                      ...
                      {r.success ? <CheckCircle className="w-4 h-4 text-emerald-600" /> : <XCircle className="w-4 h-4 text-rose-600" />}
                  ...
                  <div className="flex flex-wrap gap-2 text-[10px] font-bold text-slate-400 mt-1 border-t border-slate-100/60 pt-2.5">
                    <span className="text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded">Docs: {r.downloaded_docs ?? 0}/{r.expected_docs ?? 0}</span>
                    <span className="text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded">Iters: {r.iterations}</span>
