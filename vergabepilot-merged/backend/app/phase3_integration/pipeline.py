...
async def _try_llm_generated(
    ...
    # Pre-flight: classify the platform and (best-effort) trace the route.
    # Both are passed into the feedback loop so the generator can produce
    # a deterministic, route-following scraper instead of guessing.
    ...
    loop = await asyncio.to_thread(
        _run_loop_sync, url, llm, route_map, platform if platform != "unknown" else None,
    )
    ...
    if loop.success and loop.final_scraper and loop.final_execution:
        ...
        if moved:
            result.downloaded.extend(moved)
            # Promote to registry only if we actually got files out.
...
def _run_loop_sync(url: str, llm: LLMClient, route_map=None, platform=None):
    """Helper to run the async feedback loop from inside `to_thread`."""
