...
import sys, io
from vergabe24_token import extract_tender_id, recover_vergabe24_url
...
def portal_rewrite(url: str) -> List[str]:
    ...
    if "bieterzugang.deutsche-evergabe.de" in netloc:
        ...
        sid = None
        if m: sid = m.group(1)
        elif m2: sid = m2.group(1)
        
        if sid:
            alts.append(sid) # just the id
            alts.append(f"https://www.deutsche-evergabe.de/dashboards/DetailsDashboard/{sid}")
            alts.append(f"https://www.deutsche-evergabe.de/vergabe/detail/{sid}")
            alts.append(f"https://bieterzugang.deutsche-evergabe.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId={sid}")
    ...
    if "auftraege.bayern.de" in netloc:
        ...
        if m:
            sid = m.group(1)
            alts.append(f"https://www.auftraege.bayern.de/dashboards/dashboard_off/{sid}")
            alts.append(f"https://www.auftraege.bayern.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId={sid}")
    ...
    if "vergabe24.de" in netloc or "tender24.de" in netloc:
        ...
        if m:
            ...
            # ── NOU: încearcă europa.vergabe24.de (URL-ul care funcționează) ──
            alts.append(f"https://europa.vergabe24.de/NetServer/TenderingProcedureDetails?function=_Details&TenderOID={tid}")
            alts.append(f"https://europa.vergabe24.de/NetServer/PublicationControllerServlet?function=Detail&TWOID={tid}&PublicationType=0")
    # ── subreport.de  ─────────────────────────────────────────────────────
    if "subreport.de" in netloc:
        m = re.search(r'/E(\d+)$', path)
        if m:
            eid = m.group(1)
            alts.append(f"https://www.subreport.de/E{eid}")
...
async def load_with_recovery(page, row: dict) -> Tuple[str, str, Optional[str]]:
    ...
    async def try_url(url: str, label: str, timeout: int = 15000):
        # Setup a network listener to catch the token even if the main frame gets stuck
        sniffed_url = None
        def catch_req(req):
        ...
        try:
            ...
            # Smash cookie banner immediately to unblock JS redirects
            await dismiss_cookies(page)

            if "vergabe24.de" in url or "tender24.de" in url:
                # Poll for the token to appear, repeatedly trying to dismiss cookies
                for _ in range(12):
                ...
                # If we sniffed the token but page.url didn't update cleanly
            ...
            is_download = any(x in final_url.lower() for x in (".ashx", ".php", ".zip", ".pdf", "/download"))
            if not text or len(text) < 50:
                if is_download:
                    return "DOWNLOAD_LINK", final_url
                return None
            ...
            page.remove_listener("request", catch_req)
    ...
    if "vergabe24" in domain_check or "tender24" in domain_check:
        recovery_result = recover_vergabe24_url(normalized)
        if recovery_result["found_url"]:
            recovered_url = recovery_result["found_url"]
            log.info(f"    🔄 vergabe24 recovery: {recovered_url[:80]}")
            res = await try_url(recovered_url, "vergabe24-netserver")
            if res:
                return res[0], res[1], f"vergabe24_{recovery_result['strategy']}"

    res = await try_url(normalized, "original")
    if res: return res[0], res[1], None

    stripped = strip_documents_suffix(normalized)
    if stripped:
        res = await try_url(stripped, "strip-/documents")
        if res: return res[0], res[1], "strip_documents"

    for alt in portal_rewrite(normalized):
        res = await try_url(alt, f"portal-rewrite")
        if res: return res[0], res[1], "portal_rewrite"

    if normalized != original_url:
        res = await try_url(original_url, "original-raw")
        if res: return res[0], res[1], "original_raw"
    ...
    if found_url and found_url != normalized:
        ...
        if res: return res[0], res[1], "search_fallback"
    ...
    try:
        ...
        for link in links:
            try:
                ...
                if "evergabe" in page_domain and score == 0:
...
async def download_documents(page, tender_id, base_url,
    ...
    if "evergabe" in netloc or "auftraege.bayern.de" in netloc:
        ...
        sid = None
        if m_sid_q: sid = m_sid_q.group(1)
        elif m_sid: sid = m_sid.group(1)
        ...
        if sid and len(sid) > 10:
            if "auftraege.bayern.de" in netloc:
                portal_urls.append(f"[https://www.auftraege.bayern.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId=](https://www.auftraege.bayern.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId=){sid}")
                portal_urls.append(f"[https://bieterzugang.auftraege.bayern.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId=](https://bieterzugang.auftraege.bayern.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId=){sid}")
            else:
                portal_urls.append(f"[https://bieterzugang.deutsche-evergabe.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId=](https://bieterzugang.deutsche-evergabe.de/evergabe.bieter/DownloadTenderFiles.ashx?subProjectId=){sid}")
                portal_urls.append(f"[https://www.deutsche-evergabe.de/dashboards/DetailsDashboard/](https://www.deutsche-evergabe.de/dashboards/DetailsDashboard/){sid}")
...
def should_skip_url(url: str) -> bool:
    ...
    if re.search(r'/unterlagen/[0-9a-f-]{36}/zustellweg', low):
        return True
    return any(p in low for p in SKIP_URL_PATTERNS[2:])
    ...
    try:
        ...
        try:
            ...
            if "closed" in str(e).lower(): 
                r["err"] = "page closed during extraction"
            else: 
        ...
        if download_docs and r["status"] == "success":
            try:
                ...
                log.debug(f"    downloads failed: {e}")
                r["err"] = f"downloads: {str(e)[:100]}"

    except Exception as e:
        r["status"] = "error"
        r["err"] = str(e)[:300]
        if "closed" in str(e).lower(): pass
        else: log.error(f"    !!! error on {url[:60]}: {e}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--download-doc",       action="store_true", dest="download_docs", help=argparse.SUPPRESS)
