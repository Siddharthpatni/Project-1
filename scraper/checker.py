...
Requirements:
    pip install playwright openai requests PyPDF2 python-docx openpyxl
...
def empty_result(row: dict, domain: str) -> dict:
    ...
    return {
        ...
        "docs_relevant":   0,
        "docs_deleted":    0,
        "doc_validation":  [],         # per-doc detail: [{file, score, method, kept}]
        "url_recovery":    None,
        "err":             None,
        "ms":              0,
        "ts":              "",
...
# ═══════════════════════════════════════════════════════════════════════════
#
#  After downloading, we check each file against the tender metadata using
#  Two free methods:
#    1. Filename pattern check     — instant, free
#    2. Content keyword matching   — needs text extraction, free
#
#  Irrelevant files are deleted automatically.
# ═══════════════════════════════════════════════════════════════════════════

# Minimum total score to keep a document (out of ~8 max)
DOC_RELEVANCE_THRESHOLD = 3

# Keywords that strongly suggest a file belongs to a procurement tender
RELEVANCE_FILENAME_KEYWORDS = [
    "vergabe", "unterlag", "leistung", "ausschreibung", "angebot",
    "bekanntmachung", "gaeb", "formular", "bewerbung", "eignungs",
    "vertrag", "lv_", "lv-", "lv.", "planunterlag", "bauleistung",
    "spezifikation", "pflichtenheft", "lastenheft", "baubeschreib",
    "teilnahme", "bieter", "zuschlag", "wertung",
]

# Filename patterns that suggest junk (not tender-specific)
JUNK_FILENAME_PATTERNS = [
    "agb", "datenschutz", "impressum", "nutzungsbedingung",
    "allgemeine_geschaeft", "cookie", "newsletter", "handbuch",
    "anleitung", "tutorial", "logo", "flyer", "broschuer",
    "template", "muster_leer", "blanko",
]

# Keywords to look for inside document content
RELEVANCE_CONTENT_KEYWORDS = [
    # Procurement-specific
    "auftraggeber", "vergabestelle", "vergabeart", "verfahrensart",
    "angebotsfrist", "teilnahmefrist", "zuschlagskriterien",
    "leistungsverzeichnis", "leistungsbeschreibung", "cpv",
    "vergabenummer", "aktenzeichen", "bekanntmachung",
    "ausschreibung", "öffentliche ausschreibung",
    "beschränkte ausschreibung", "verhandlungsverfahren",
    "offenes verfahren", "freihändige vergabe",
    "bieter", "auftragnehmer", "los ", "lose ",
    # Construction-specific (common in German tenders)
    "bauleistung", "rohbau", "tiefbau", "hochbau",
    "vob", "vob/a", "uvgo", "gwb",
]


def _extract_text_from_pdf(filepath: str, max_chars: int = 5000) -> str:
    ...
    try:
        ...
        log.debug(f"    PDF text extraction failed for {filepath}: {exc}")
        return ""


def _extract_text_from_docx(filepath: str, max_chars: int = 5000) -> str:
    ...
    try:
        ...
        for para in doc.paragraphs:
            ...
            chars_so_far += len(para.text)
            if chars_so_far >= max_chars:
                break
        # Also grab table content (tender specs are often in tables)
        if chars_so_far < max_chars:
            for table in doc.tables:
                for row in table.rows:
                    for cell in row.cells:
                        text_parts.append(cell.text)
                        chars_so_far += len(cell.text)
                        if chars_so_far >= max_chars:
                            break
                    if chars_so_far >= max_chars:
                        break
        ...
        log.debug(f"    DOCX text extraction failed for {filepath}: {exc}")
        return ""


def _extract_text_from_xlsx(filepath: str, max_chars: int = 5000) -> str:
    """Extract text from an XLSX file by reading all cell values."""
    try:
        ...
        log.debug(f"    XLSX text extraction failed for {filepath}: {exc}")
        return ""


def _inspect_zip_contents(filepath: str) -> Tuple[str, List[str]]:
    """
    Inspect a ZIP file: return (concatenated filenames as text, list of filenames).
    We check whether the files inside the ZIP look procurement-related.
    """
    try:
        with zipfile.ZipFile(filepath, "r") as zf:
            names = zf.namelist()
            # Build a pseudo-text from all filenames for keyword matching
            text = " ".join(names)
            return text, names
    except Exception as exc:
        log.debug(f"    ZIP inspection failed for {filepath}: {exc}")
        return "", []


def _score_zip_inner_files(filepath: str) -> int:
    """
    Score a ZIP by checking whether its inner filenames look procurement-related.
    Returns 0–3 based on how many inner files have procurement keywords.
    """
    _, names = _inspect_zip_contents(filepath)
    if not names:
        return 0

    relevant_count = 0
    for name in names:
        inner_score = score_filename(name)
        if inner_score >= 2:
            relevant_count += 1
...
def _extract_document_text(filepath: str) -> str:
    ...
    if ext == ".pdf":
        ...
        # python-docx can't read old .doc; try anyway, usually fails gracefully
        return _extract_text_from_docx(filepath)
    elif ext in (".xlsx", ".xls"):
        return _extract_text_from_xlsx(filepath)
    elif ext == ".zip":
        text, _ = _inspect_zip_contents(filepath)
        return text
    else:
        # Unsupported format — can't extract text, rely on filename score
        return ""


def score_filename(filename: str) -> int:
    """
    Score a filename for procurement relevance (0–3).
      0 = looks like junk
      1 = generic / neutral
      2 = has procurement-related words
      3 = strong procurement signal
    """
    name_lower = filename.lower()

    # Known junk → immediate 0
    if any(junk in name_lower for junk in JUNK_FILENAME_PATTERNS):
        return 0

    # Count how many procurement keywords appear in the filename
    hits = sum(1 for kw in RELEVANCE_FILENAME_KEYWORDS if kw in name_lower)
    ...
    if hits >= 2:
        ...
        # Even a generic "doc_1_abc123.pdf" gets 1 (benefit of the doubt)
        return 1


def score_content_keywords(doc_text: str, tender_meta: dict) -> int:
    """
    Score document content against tender metadata and procurement keywords (0–5).

    tender_meta should contain keys like: title, authority, ref_num, cpv, id
    """
    ...
    # Check if the tender's own identifiers appear in the document
    for key in ("ref_num", "id", "cpv"):
        val = tender_meta.get(key)
        if val and len(str(val)) > 3 and str(val).lower() in text_lower:
            score += 2  # Strong match — the doc contains the tender's own ID

    # Check if authority or title words appear
    for key in ("authority", "title"):
        val = tender_meta.get(key)
        if val:
            # Split into words, check if at least 2 significant words match
            words = [w for w in str(val).lower().split() if len(w) > 3]
            matched = sum(1 for w in words if w in text_lower)
            if matched >= 2:
                score += 1

    # Check for generic procurement vocabulary in the document
    generic_hits = sum(1 for kw in RELEVANCE_CONTENT_KEYWORDS if kw in text_lower)
    if generic_hits >= 5:
        score += 2
    elif generic_hits >= 2:
        score += 1
...
def validate_documents(folder: str, saved_files: List[str],
                       tender_meta: dict) -> Tuple[List[str], List[dict]]:
    """
    Validate each downloaded document for relevance to the tender.

    Two-layer check (zero API cost):
      1. Filename pattern scoring    (instant)
      2. Content keyword matching    (needs text extraction from PDF/DOCX/XLSX/ZIP)

    Returns:
      kept_files  — filenames that passed validation
      details     — per-file validation report
    """
    kept_files = []
    details = []
    ...
    for filename in saved_files:
        ...
        fn_score = score_filename(filename)
        file_detail["filename_score"] = fn_score

        # If filename is clearly junk (score 0), delete immediately
        if fn_score == 0:
            file_detail["total_score"] = 0
            file_detail["method"] = "filename_junk"
            _delete_irrelevant_doc(filepath, filename, 0)
            details.append(file_detail)
            continue

        # ── Layer 2: content keyword matching ──
        doc_text = _extract_document_text(filepath)
        content_score = score_content_keywords(doc_text, tender_meta)
        file_detail["content_score"] = content_score
        file_detail["method"] = "keyword"

        combined = fn_score + content_score

        # ── Layer 2b: ZIP inner-file scoring ──
        if Path(filename).suffix.lower() == ".zip":
            zip_inner_score = _score_zip_inner_files(filepath)
            if zip_inner_score > 0:
                combined += zip_inner_score
                file_detail["content_score"] += zip_inner_score
                log.debug(f"    ZIP inner-file bonus: +{zip_inner_score}pt")

        file_detail["total_score"] = combined

        # ── Decision: keep or delete ──
        if combined >= DOC_RELEVANCE_THRESHOLD:
            file_detail["kept"] = True
            kept_files.append(filename)
            STATS["docs_relevant"] += 1
            log.info(f"    ✓ relevant ({combined}pt): {filename}")
        else:
            _delete_irrelevant_doc(filepath, filename, combined)

        details.append(file_detail)

    return kept_files, details


def _delete_irrelevant_doc(filepath: str, filename: str, score: int):
    """Delete a document that failed relevance validation."""
    try:
        os.remove(filepath)
        STATS["docs_deleted"] += 1
        log.info(f"    ✗ deleted ({score}pt): {filename}")
    except Exception as exc:
        log.debug(f"    failed to delete {filename}: {exc}")
        ...
        if download_docs and result["status"] == "success":
            try:
                ...
                if docs:
                    ...
                    kept, validation_details = validate_documents(
                        doc_folder, docs, tender_meta,
                    )

                    result["downloaded_docs"] = kept
                    result["docs_validated"] = len(docs)
                    result["docs_relevant"] = len(kept)
                    result["docs_deleted"] = len(docs) - len(kept)
                    result["doc_validation"] = validation_details

                    if len(kept) < len(docs):
                        log.info(
                            f"    validation: {len(kept)}/{len(docs)} kept, "
                            f"{len(docs) - len(kept)} deleted"
                        )

                    # Clean up empty folder if all docs were deleted
                    if not kept and os.path.isdir(doc_folder):
                        try:
...
CSV_COLUMNS = [
    ...
    "downloaded_docs", "docs_validated", "docs_relevant", "docs_deleted",
    "url_recovery", "err", "ms", "ts",
]
...
def save_results(results: list, output_json: str) -> str:
    ...
    with open(csv_path, "w", encoding="utf-8", newline="") as f:
        ...
        for rec in results:
            ...
            row.pop("doc_validation", None)  # too detailed for CSV; kept in JSON
            writer.writerow(row)
...
def print_summary(results: list, use_llm: bool, download_docs: bool):
    ...
    if download_docs:
        ...
        if STATS["docs_validated"]:
            kept = STATS["docs_relevant"]
            deleted = STATS["docs_deleted"]
            checked = STATS["docs_validated"]
            pct = 100 * kept / checked if checked else 0
            print(f"  docs validated: {checked}")
            print(f"  docs relevant:  {kept}  ({pct:.1f}%)")
            print(f"  docs deleted:   {deleted}")
